...
for (i in 1:num_partes) {
  df <- DBI::dbGetQuery(
    ...
    statement = paste0("SELECT DISTINCT DATA, NUM_CUPOM, LOJA, VALORIDENTCLIENTE 
                        ...
                        WHERE DATA BETWEEN '2023-02-01' AND '2023-02-28' AND LOJA = 6 AND
...
writexl::write_xlsx(
  list(clientes_matoso_bairro_loja = df_consolidado),
  'output_dashboard_crm_interno/6.2.analise_clientes_matoso_bairro_loja.xlsx'
)
